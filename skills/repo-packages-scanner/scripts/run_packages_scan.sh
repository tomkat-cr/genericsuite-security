#!/usr/bin/env bash
# run_packages_scan.sh - Verify detection works, then scan a corpus.
#
# Usage:
#   ./run_packages_scan.sh --corpus /path/to/corpus.json
#   ./run_packages_scan.sh --org tomkat-cr            # builds the corpus first
#   ./run_packages_scan.sh --local .                  # single checkout, lint mode
#
# Exit: 0 clean at the threshold, 1 findings, 2 error.
#
# NO BASH ARRAYS ANYWHERE IN THIS FILE. macOS ships bash 3.2, where "${a[@]}"
# on an EMPTY array under `set -u` aborts with "unbound variable" - and worse,
# a crash inside a command substitution can look like an exit-1 scan verdict
# rather than a broken run. See repo-docker-scanner/scripts/run_docker_scan.sh,
# which shipped and then fixed exactly that bug; this driver is written to the
# same rule from the start rather than repeating it.
set -uo pipefail
...
# below, so the report can state what actually produced it. --org/--include/
# --branch are resolved into a --corpus path long before scan_packages.py ever
# runs, so its own argv alone would only show the internal --corpus ...
# --sarif call. POSIX single-quote escaping done by hand (no bash arrays).
PACKAGES_SCAN_INVOKED_CMD="./scripts/run_packages_scan.sh"
for _a in "$@"; do
  _q="$(printf '%s' "$_a" | sed "s/'/'\\\\''/g")"
  PACKAGES_SCAN_INVOKED_CMD="$PACKAGES_SCAN_INVOKED_CMD '$_q'"
done
export PACKAGES_SCAN_INVOKED_CMD
unset _a _q
...
if [ "${PACKAGES_SCAN_SKIP_SELFTEST:-0}" = "1" ]; then
  echo "### Step 1: SELF-TEST SKIPPED (PACKAGES_SCAN_SKIP_SELFTEST=1)"
...
echo
echo "### Step 3: scan for unpinned dependencies and Actions"
REPORT="$(python3 "$SKILL_DIR/scripts/scan_packages.py" --corpus "$CORPUS" --sarif "$@")"
RC=$?
...
echo "  This is not the same as 'nothing is unpinned': see the report's"
echo "  blind-spot section."
exit 0
